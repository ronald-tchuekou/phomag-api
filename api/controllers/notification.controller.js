...
exports.createNotification = async (req, res) => {
   try {
      ...
      else user = await UserModel.getUserWhere({ user_id: data.receiver_id })
      const notification_token = user.notification_token

      if (notification_token) await sendNotificationPush([notification_token], data.title, data.message, data)
...
exports.markAsRead = async (req, res) => {
   ...
   const update = async (item) => {
      response.push(await NotificationModel.updateNotification({ is_read: true }, item))
   }
   try {
      if (req.body.notification_ids) req.body.notification_ids.forEach(update)
      res.json(response)
   } catch (e) {
      console.log(e)
      res.status(400).json({
